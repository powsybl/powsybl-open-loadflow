...
The AC calculations are based on Newton-Raphson iterations. The DC calculations are based on direct current linear approximation. Open Load Flow relies on:
 - a fast and robust convergence, based on [KLU](http://faculty.cse.tamu.edu/davis/suitesparse.html) numerical solver, that supports both dense and sparse matrices.
 - a distributed slack (on generation or on loads or on conform loads); it also supports non distributed slack; in both cases, the slack bus selection is configurable as explained [here](https://www.powsybl.org/pages/documentation/simulation/powerflow/openlf.html#parameters). The distributed slack that can be configured by country.
 - a support of generators' active and reactive power limits, included the support of reactive capability curves.
 - 5 starting point modes: flat, warm, only angles initialization based on a DC load flow, only voltages initialization based on a specific initializer, or both angles and voltages initialization based on the two previous methods.
 - a support of non impedant branches. As we do not support loops of non impedant branches, a minimal number of non impedant lines will be treated with a minimal impedance.
 ...
 - a local and remote phase control: phase tap changers can regulate active power flows or limits currents at given terminals.
 ...
 - a remote reactive power control by a single generator connected on a bus.
...
### Security analysis implementation 
 ...
 - The active and reactive power flows on branches, angle or voltage at buses can be monitored after the base case and after each contingency.

### Sensitivity analysis implementation 

 Open Load Flow both supports AC and DC calculations. Even if it comes from the same powsybl-core API, the calculations behind are radically different. The AC calculation is based on the same principles than the AC security analysis, where a network listener automatically updates the equation system. The DC calculation is fully documented [here](https://www.powsybl.org/pages/documentation/simulation/sensitivity/openlf.html).
...
- Hybrid shared controls modelling with different kinds of equipments as generators, static var compensators, tap changers and shunts;
...
- Increasing performances of the AC security analysis implementation.
- Increasing performances of the AC sensitivity analysis implementation.
- Repository reorganisation in a multi-modules Maven project that could be renamed PowSyBl open Simulator with:   
        * powsybl-open-simulator-matrix, transfer of com.powsybl.math.matrix from powsybl-core,  
        * powsybl-open-simulator-network with the ```LfNetwork``` renamed ```OsNetwork```,  
        * powsybl-open-simulator-core with the equations and maybe more,  
        * powsybl-open-loadflow with implementations of load flow, security analysis and sensitivity analysis.   
