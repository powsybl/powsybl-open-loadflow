...
public class LfContingency {
    ...
    public void apply(LoadFlowParameters.BalanceType balanceType) {
        ...
        for (var e : lostLoads.entrySet()) {
            ...
            load.setTargetP(load.getTargetP() - getUpdatedLoadP0(load, balanceType, shift.getActive(), shift.getVariableActive(), lostLoad.getInitialFictitiousP0()));
    ...
    private static double getUpdatedLoadP0(LfLoad lfLoad, LoadFlowParameters.BalanceType balanceType, double initialP0, double initialVariableActivePower, double initialFictitiousP0) {
        ...
        if (!lfLoad.isFictitious() && lfLoad.getOriginalLoadCount() > 0) {
            if (balanceType == LoadFlowParameters.BalanceType.PROPORTIONAL_TO_LOAD) {
                factor = (Math.abs(initialP0) - Math.abs(initialFictitiousP0)) / lfLoad.getAbsVariableTargetP();
