...
public class VoltageControl<T extends LfElement> extends Control {
    ...
    public static final List<String> VOLTAGE_CONTROL_PRIORITIES = List.of(Type.VOLTAGE_SOURCE_CONVERTER.name(), Type.GENERATOR.name(), Type.TRANSFORMER.name(), Type.SHUNT.name());
    ...
    public static boolean checkTargetV(double targetV, double nominalV, LfNetworkParameters parameters) {
        return nominalV <= parameters.getMinNominalVoltageTargetVoltageCheck()
            || isTargetVoltagePlausible(targetV, parameters.getMinPlausibleTargetVoltage(), parameters.getMaxPlausibleTargetVoltage());
    }
