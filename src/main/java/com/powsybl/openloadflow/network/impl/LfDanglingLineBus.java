    ...
    public LfDanglingLineBus(DanglingLine danglingLine) {
        ...
        if (danglingLine.getGeneration() != null) {
            if (danglingLine.getGeneration().isVoltageRegulationOn()) {
                this.targetV = danglingLine.getGeneration().getTargetV();
                this.voltageControl = true;
                this.voltageControlCapacility = true;
            } else {
                if (!Double.isNaN(danglingLine.getGeneration().getTargetQ())) {
                    generationTargetQ += danglingLine.getGeneration().getTargetQ();
                }
            }
            if (!Double.isNaN(danglingLine.getGeneration().getTargetP())) {
