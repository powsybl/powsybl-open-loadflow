...
public class LfDanglingLineBranch extends AbstractFictitiousLfBranch {
    ...
    public List<LfLimit> getLimits1(final LimitType type) {
        switch (type) {
            case ACTIVE_POWER:
                return getLimits1(type, danglingLine.getNullableActivePowerLimits());
            case APPARENT_POWER:
                return getLimits1(type, danglingLine.getNullableApparentPowerLimits());
            case CURRENT:
                return getLimits1(type, danglingLine.getNullableCurrentLimits());
            case VOLTAGE:
            default:
