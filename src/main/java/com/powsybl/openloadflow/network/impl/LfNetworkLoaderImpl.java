...
public class LfNetworkLoaderImpl implements LfNetworkLoader<Network> {
            ...
            if (!voltageControlGenerators.isEmpty()) {
                ...
                voltageControlGenerators.stream().skip(1).forEach(lfGenerator -> {
                    ...
                    if (controlledBus.getNum() != generatorControlledBus.getNum()) {
                        ...
                        addToVoltageControl(controllerBusCopy, generatorControlledBus, lfGenerator.getTargetV(), voltageRemoteControl, voltagePerReactivePowerControl, voltageControls);
                    }
                ...
                addToVoltageControl(controllerBus, controlledBus, controllerTargetV, voltageRemoteControl, voltagePerReactivePowerControl, voltageControls);
            }
        }
    ...
    private static void addToVoltageControl(LfBus controllerBus, LfBus controlledBus, double controllerTargetV, boolean voltageRemoteControl,
                                            boolean voltagePerReactivePowerControl, List<VoltageControl> voltageControls) {
    ...
    private static void fixAllVoltageControls(LfNetwork lfNetwork, boolean minImpedance, boolean transformerVoltageControl,
        ...
        if (!minImpedance) {
            ...
            List<Set<LfBus>> connectedSets = new ConnectivityInspector<>(lfNetwork.createZeroImpedanceSubGraph(dc)).connectedSets();
    ...
    private static LfNetwork create(int numCC, int numSC, List<Bus> buses, List<Switch> switches, LfNetworkParameters parameters, Reporter reporter) {
        ...
        if (!parameters.isMinImpedance()) {
            ...
            Graph<LfBus, LfBranch> zeroImpedanceSubGraph = lfNetwork.createZeroImpedanceSubGraph(parameters.isDc());
