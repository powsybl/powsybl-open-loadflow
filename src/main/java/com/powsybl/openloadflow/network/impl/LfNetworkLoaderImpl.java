...
public class LfNetworkLoaderImpl implements LfNetworkLoader {
    ...
    private static void createBranches(LfNetwork lfNetwork, LoadingContext loadingContext, LfNetworkLoadingReport report) {
        ...
        for (DanglingLine danglingLine : loadingContext.danglingLines) {
            ...
            DanglingLineData danglingLineData = new DanglingLineData(danglingLine);
            lfBus2.setV(danglingLineData.getDummyBusU() / lfBus2.getNominalV());
            lfBus2.setAngle(Math.toDegrees(danglingLineData.getDummyBusTheta()));
        }
        ...
        for (ThreeWindingsTransformer t3wt : loadingContext.t3wtSet) {
            ...
            TwtData twtdata = new TwtData(t3wt, phaseAngleClock2, phaseAngleClock3, epsilonX, applyReactanceCorrection,
                    twtSplitShuntAdmittance);

            System.err.printf("Twt Id %f %f %n", twtdata.getStarU() / lfBus0.getNominalV(), Math.toDegrees(twtdata.getStarTheta()));
            lfBus0.setV(twtdata.getStarU() / lfBus0.getNominalV());
            lfBus0.setAngle(Math.toDegrees(twtdata.getStarTheta()));
        }
    }
