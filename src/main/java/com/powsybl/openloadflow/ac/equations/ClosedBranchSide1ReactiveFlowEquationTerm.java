...
public class ClosedBranchSide1ReactiveFlowEquationTerm extends AbstractClosedBranchAcFlowEquationTerm {
    ...
    public void update(double[] x, BranchVector vec) {
        ...
        q1 = r1 * v1 * (-vec.b1[branchNum] * r1 * v1 + vec.y[branchNum] * r1 * v1 * vec.cosKsi[branchNum] - vec.y[branchNum] * R2 * v2 * cosTheta);
        dq1dv1 = r1 * (-2 * vec.b1[branchNum] * r1 * v1 + 2 * vec.y[branchNum] * r1 * v1 * vec.cosKsi[branchNum] - vec.y[branchNum] * R2 * v2 * cosTheta);
        dq1dv2 = -vec.y[branchNum] * r1 * R2 * v1 * cosTheta;
        dq1dph1 = -vec.y[branchNum] * r1 * R2 * v1 * v2 * sinTheta;
        dq1dph2 = -dq1dph1;
        if (a1Var != null) {
            dq1da1 = dq1dph1;
        }
        if (r1Var != null) {
            dq1dr1 = v1 * (2 * r1 * v1 * (-vec.b1[branchNum] + vec.y[branchNum] * vec.cosKsi[branchNum]) - vec.y[branchNum] * R2 * v2 * cosTheta);
        }
    }
