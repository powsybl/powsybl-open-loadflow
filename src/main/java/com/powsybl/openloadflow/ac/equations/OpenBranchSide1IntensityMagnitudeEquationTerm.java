...
public class OpenBranchSide1IntensityMagnitudeEquationTerm extends AbstractOpenSide1BranchAcFlowEquationTerm {
    ...
    public void update(double[] x) {
        ...
        double reI2 = R2 * w2 * (gres * cosPh2 - bres * sinPh2) * NORMALIZATION_FACTOR;
        double imI2 = R2 * w2 * (gres * sinPh2 + bres * cosPh2) * NORMALIZATION_FACTOR;
        i2 = Math.hypot(reI2, imI2);

        double dreI2dv2 = R2 * R2 * (gres * cosPh2 - bres * sinPh2) * NORMALIZATION_FACTOR;

        double dimI2dv2 = R2 * R2 * (gres * sinPh2 + bres * cosPh2) * NORMALIZATION_FACTOR;
