 ...
 */
public class ApproximateLoadEquationTerm extends AbstractNamedEquationTerm<AcVariableType, AcEquationType> {
    ...
    public ApproximateLoadEquationTerm(LfBus bus, VariableSet<AcVariableType> variableSet, boolean isActive, int sequenceNum) {
        super(true);
    ...
    public static double pq(boolean isActive, int sequenceNum, ApproximateLoadEquationTerm eqTerm, double vo, double pho, double vd, double phd, double vi, double phi) {
        ...
        //        <-------------------------------------------->
        //                          term T0
        // We use a simplified approach : Sa/Va, Sb/Vb, Sc/Vc are initialized as constants, with balanced Va, Vb, Vc
        // Load is then considered as constant current injection as a first approximation
        ...
        // build of Vabc Balanced vector as a default value for Sabc/Vabc
        DenseMatrix mVfortescueBalanced = getCartesianMatrix(0, 0, 1, 0, 0, 0, true);
        DenseMatrix mVabcBalanced = getFortescueMatrix().times(mVfortescueBalanced);

        // build  1/Vabc square matrix with balanced a, b, c voltages (approximation)
        DenseMatrix mInvVabcBalanced = getInvVabcSquare(mVabcBalanced.get(0, 0), mVabcBalanced.get(1, 0), mVabcBalanced.get(2, 0), mVabcBalanced.get(3, 0), mVabcBalanced.get(4, 0), mVabcBalanced.get(5, 0), eqTerm);
        ...
        DenseMatrix m0T0 = mInvVabcBalanced.times(mSabc3);
    ...
    public static double dpq(boolean isActive, int sequenceNum, ApproximateLoadEquationTerm eqTerm, Variable<AcVariableType> derVariable, double vo, double pho, double vd, double phd, double vi, double phi) {
        // We derivate the PQ formula with complex matrices, given the approx in pq() term T2 will be neglected
        //
        ...
        // build of Vabc Balanced unit vector as a default value for Sabc/Vabc
        DenseMatrix mVfortescueBalanced = getCartesianMatrix(0, 0, 1, 0, 0, 0, true);
        DenseMatrix mVabcBalanced = getFortescueMatrix().times(mVfortescueBalanced);
        // build  1/Vabc square matrix with balanced a, b, c voltages (approximation)
        DenseMatrix mInvVabcBalanced = getInvVabcSquare(mVabcBalanced.get(0, 0), mVabcBalanced.get(1, 0), mVabcBalanced.get(2, 0), mVabcBalanced.get(3, 0), mVabcBalanced.get(4, 0), mVabcBalanced.get(5, 0), eqTerm);
        ...
        DenseMatrix m0T1 = mInvVabcBalanced.times(mSabc3);
        ...
        if (isActive && sequenceNum == 0) {
            return mT1.get(0, 0); // Po
        } else if (!isActive && sequenceNum == 0) {
            return mT1.get(1, 0); // Qo
        } else if (isActive && sequenceNum == 1) {
            return mT1.get(2, 0); // Pd
        } else if (!isActive && sequenceNum == 1) {
            return mT1.get(3, 0); // Qd
        } else if (isActive && sequenceNum == 2) {
            return mT1.get(4, 0); // Pi
        } else if (!isActive && sequenceNum == 2) {
            return mT1.get(5, 0); // Qi
        } else {
            throw new IllegalStateException("Unknow variable at bus : " + eqTerm.bus.getId());
        }
    }
    ...
    public static DenseMatrix getInvVabcSquare(double vAx, double vAy, double vBx, double vBy, double vCx, double vCy, ApproximateLoadEquationTerm eqTerm) {
