...
public class AcEquationSystemCreator {
    ...
    private void createVoltageControlEquations(EquationSystem<AcVariableType, AcEquationType> equationSystem, BranchAcDataVector branchAcDataVector) {
        for (LfBus bus : network.getBuses()) {
            createGeneratorVoltageControlEquations(bus, equationSystem, branchAcDataVector);
    ...
    private void createGeneratorVoltageControlEquations(LfBus bus,
                                                        EquationSystem<AcVariableType, AcEquationType> equationSystem,
                                                        BranchAcDataVector branchAcDataVector) {
                ...
                .ifPresent(voltageControl -> {
                    if (bus.isGeneratorVoltageControlled()) {
                        if (voltageControl.isLocalControl()) {
                            createGeneratorLocalVoltageControlEquation(bus, equationSystem, branchAcDataVector);
                        } else {
                            // create reactive power distribution equations at voltage controller buses
                            createGeneratorReactivePowerDistributionEquations(voltageControl, equationSystem, creationParameters, branchAcDataVector);
                        }
    ...
    private void createGeneratorLocalVoltageControlEquation(LfBus bus,
                                                            EquationSystem<AcVariableType, AcEquationType> equationSystem, BranchAcDataVector branchAcDataVector) {
        if (bus.hasGeneratorsWithSlope()) {
            ...
            equationSystem.getEquation(bus.getNum(), AcEquationType.BUS_TARGET_V).orElseThrow()
                    .addTerms(createReactiveTerms(bus, equationSystem.getVariableSet(), creationParameters, branchAcDataVector)
                            .stream()
        ...
        if (bus1 != null && bus2 != null) {
            branch.getGeneratorReactivePowerControl().ifPresent(rpc -> {
                EquationTerm<AcVariableType, AcEquationType> q = rpc.getControlledSide() == TwoSides.ONE
                        ? new ClosedBranchSide1ReactiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector)
                        : new ClosedBranchSide2ReactiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
                equationSystem.createEquation(branch, AcEquationType.BRANCH_TARGET_Q)
                        .addTerm(q);
                createGeneratorReactivePowerDistributionEquations(rpc, equationSystem, creationParameters, branchAcDataVector);
    ...
    private static void createGeneratorReactivePowerDistributionEquations(Control control, EquationSystem<AcVariableType, AcEquationType> equationSystem,
                                                                          AcEquationSystemCreationParameters creationParameters, BranchAcDataVector branchAcDataVector) {
        ...
        for (LfBus controllerBus : controllerBuses) {
            ...
            Equation<AcVariableType, AcEquationType> zero = equationSystem.createEquation(controllerBus, AcEquationType.DISTR_Q)
                    .addTerms(createReactiveTerms(controllerBus, equationSystem.getVariableSet(), creationParameters, branchAcDataVector).stream()
            ...
            for (LfBus otherControllerBus : controllerBuses) {
                if (otherControllerBus != controllerBus) {
                    zero.addTerms(createReactiveTerms(otherControllerBus, equationSystem.getVariableSet(), creationParameters, branchAcDataVector).stream()
    ...
    public static void recreateReactivePowerDistributionEquations(LfNetwork network, GeneratorVoltageControl voltageControl,
        ...
        if (!voltageControl.isLocalControl()) {
            createGeneratorReactivePowerDistributionEquations(voltageControl, equationSystem, parameters, branchAcDataVector);
        }
        updateGeneratorVoltageControl(voltageControl, equationSystem);
    }
            ...
            if (branch.isZeroImpedance(LoadFlowModel.AC)) {
                ...
                if (branch.getBus1() == controllerBus) {
                    ...
                    if (otherSideBus != null) {
                        q = new ClosedBranchSide1ReactiveFlowEquationTerm(branch, controllerBus, otherSideBus, variableSet, deriveA1, deriveR1, branchAcDataVector);
                        branch.addAdditionalClosedQ1(q);
                        if (branch.isDisconnectionAllowedSide2()) {
                            openQ = new OpenBranchSide2ReactiveFlowEquationTerm(branch, controllerBus, variableSet, branchAcDataVector);
                            branch.addAdditionalOpenQ1(openQ);
                        }
                    } else {
                        q = new OpenBranchSide2ReactiveFlowEquationTerm(branch, controllerBus, variableSet, branchAcDataVector);
                        branch.addAdditionalOpenQ1(q);
                    }
                } else {
                    LfBus otherSideBus = branch.getBus1();
                    if (otherSideBus != null) {
                        q = new ClosedBranchSide2ReactiveFlowEquationTerm(branch, otherSideBus, controllerBus, variableSet, deriveA1, deriveR1, branchAcDataVector);
                        branch.addAdditionalClosedQ2(q);
                        if (branch.isDisconnectionAllowedSide1()) {
                            openQ = new OpenBranchSide1ReactiveFlowEquationTerm(branch, controllerBus, variableSet, branchAcDataVector);
                            branch.addAdditionalOpenQ2(openQ);
                        }
                    } else {
                        q = new OpenBranchSide1ReactiveFlowEquationTerm(branch, controllerBus, variableSet, branchAcDataVector);
        ...
        if (branch.isPhaseControlled()) {
            ...
            if (phaseControl.getMode() == Mode.CONTROLLER) {
                ...
                EquationTerm<AcVariableType, AcEquationType> p = phaseControl.getControlledSide() == TwoSides.ONE
                        ? new ClosedBranchSide1ActiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector)
                        : new ClosedBranchSide2ActiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
    ...
    protected void createImpedantBranch(LfBranch branch, LfBus bus1, LfBus bus2,
        ...
        boolean deriveR1 = isDeriveR1(branch);
        if (bus1 != null && bus2 != null) {
            closedP1 = new ClosedBranchSide1ActiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
            closedQ1 = new ClosedBranchSide1ReactiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
            closedP2 = new ClosedBranchSide2ActiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
            closedQ2 = new ClosedBranchSide2ReactiveFlowEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
            closedI1 = new ClosedBranchSide1CurrentMagnitudeEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
            closedI2 = new ClosedBranchSide2CurrentMagnitudeEquationTerm(branch, bus1, bus2, equationSystem.getVariableSet(), deriveA1, deriveR1, branchAcDataVector);
            if (branch.isDisconnectionAllowedSide1()) {
                openP2 = new OpenBranchSide1ActiveFlowEquationTerm(branch, bus2, equationSystem.getVariableSet(), branchAcDataVector);
                openQ2 = new OpenBranchSide1ReactiveFlowEquationTerm(branch, bus2, equationSystem.getVariableSet(), branchAcDataVector);
                openI2 = new OpenBranchSide1CurrentMagnitudeEquationTerm(branch, bus2, equationSystem.getVariableSet(), branchAcDataVector);
            }
            if (branch.isDisconnectionAllowedSide2()) {
                openP1 = new OpenBranchSide2ActiveFlowEquationTerm(branch, bus1, equationSystem.getVariableSet(), branchAcDataVector);
                openQ1 = new OpenBranchSide2ReactiveFlowEquationTerm(branch, bus1, equationSystem.getVariableSet(), branchAcDataVector);
                openI1 = new OpenBranchSide2CurrentMagnitudeEquationTerm(branch, bus1, equationSystem.getVariableSet(), deriveR1, branchAcDataVector);
            }
            p1 = closedP1;
            q1 = closedQ1;
            i1 = closedI1;
        ...
        createGeneratorReactivePowerControlBranchEquation(branch, bus1, bus2, equationSystem, deriveA1, deriveR1, branchAcDataVector);

        createTransformerPhaseControlEquations(branch, bus1, bus2, equationSystem, deriveA1, deriveR1, branchAcDataVector);
    ...
    private void createImpedantBranchEquations(LfBranch branch,
        ...
        if (branch.isZeroImpedance(LoadFlowModel.AC)) {
            ...
            createImpedantBranch(branch, branch.getBus1(), branch.getBus2(), equationSystem, branchAcDataVector);
        }
    }

    private void createBranchesEquations(EquationSystem<AcVariableType, AcEquationType> equationSystem, BranchAcDataVector branchAcDataVector) {
        for (LfBranch branch : network.getBranches()) {
            createImpedantBranchEquations(branch, equationSystem, branchAcDataVector);
        }
    }
        ...
        for (LfBranch branch : bus.getBranches()) {
            if (branch.isZeroImpedance(LoadFlowModel.AC)) {
                ...
                if (branch.getBus1() == bus) {
                    LfBus otherSideBus = branch.getBus2();
                    if (otherSideBus != null) {
                        p = new ClosedBranchSide1ActiveFlowEquationTerm(branch, bus, otherSideBus, variableSet, deriveA1, deriveR1, branchAcDataVector);
                        branch.addAdditionalClosedP1(p);
                        if (branch.isDisconnectionAllowedSide2()) {
                            openP = new OpenBranchSide2ActiveFlowEquationTerm(branch, bus, variableSet, branchAcDataVector);
                            branch.addAdditionalOpenP1(openP);
                        }
                    } else {
                        p = new OpenBranchSide2ActiveFlowEquationTerm(branch, bus, variableSet, branchAcDataVector);
                        branch.addAdditionalOpenP1(p);
                    }
                } else {
                    LfBus otherSideBus = branch.getBus1();
                    if (otherSideBus != null) {
                        p = new ClosedBranchSide2ActiveFlowEquationTerm(branch, otherSideBus, bus, variableSet, deriveA1, deriveR1, branchAcDataVector);
                        branch.addAdditionalClosedP2(p);
                        if (branch.isDisconnectionAllowedSide1()) {
                            openP = new OpenBranchSide1ActiveFlowEquationTerm(branch, bus, variableSet, branchAcDataVector);
                            branch.addAdditionalOpenP2(openP);
                        }
                    } else {
                        p = new OpenBranchSide1ActiveFlowEquationTerm(branch, bus, variableSet, branchAcDataVector);
    ...
    private void createMultipleSlackBusesEquations(EquationSystem<AcVariableType, AcEquationType> equationSystem, BranchAcDataVector branchAcDataVector) {
        ...
        if (slackBuses.size() > 1) {
            ...
            for (int i = 1; i < slackBuses.size(); i++) {
                ...
                equationSystem.createEquation(slackBus, AcEquationType.BUS_DISTR_SLACK_P)
                        .addTerms(createActiveInjectionTerms(firstSlackBus, equationSystem.getVariableSet(), branchAcDataVector).stream()
                                .map(EquationTerm::minus)
                                .toList())
                        .addTerms(createActiveInjectionTerms(slackBus, equationSystem.getVariableSet(), branchAcDataVector));
                // to update open/close terms activation
    ...
    public EquationSystem<AcVariableType, AcEquationType> create() {
        ...
        createBranchesEquations(equationSystem, branchAcDataVector);

        for (LfHvdc hvdc : network.getHvdcs()) {
            createHvdcAcEmulationEquations(hvdc, equationSystem);
        }

        createVoltageControlEquations(equationSystem, branchAcDataVector);

        EquationSystemPostProcessor.findAll().forEach(pp -> pp.onCreate(equationSystem));

        network.addListener(LfNetworkListenerTracer.trace(new AcEquationSystemUpdater(equationSystem, creationParameters, branchAcDataVector)));
