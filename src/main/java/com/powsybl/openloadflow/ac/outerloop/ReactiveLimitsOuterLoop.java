...
public class ReactiveLimitsOuterLoop implements AcOuterLoop {
     ...
     */
    private static void checkPqBus(LfBus controllerCapableBus, List<PqToPvBus> pqToPvBuses, List<LfBus> busesWithUpdatedQLimits, double maxReactivePowerMismatch) {
        ...
        for (LfBus bus : context.getNetwork().getBuses()) {
            if (bus.isGeneratorVoltageControlEnabled() && !bus.isDisabled()) {
                ...
                if (!bus.hasGeneratorsWithSlope()) {
                    checkPqBus(bus, pqToPvBuses, busesWithUpdatedQLimits, maxReactivePowerMismatch);
                } else {
