...
public class RandomWorkload extends AbstractGenerateWorkload {
    ...
    protected void generateOperationsImpl(BufferedWriter bw) throws IOException {
        ...
        if (computeSd > 0) {
            if (0 != total % computeSd) {
                throw new RuntimeException("the total number of update should be a multiple of computeSd");
            }
            ...
            if (0 != Arrays.stream(opRemaining).sum()) {
                throw new RuntimeException("failed to use all operations");
            }
        } else {
