...
public class DcLoadFlowEngine implements LoadFlowEngine<DcVariableType, DcEquationType, DcLoadFlowParameters, DcLoadFlowResult> {
    ...
    public static void distributeSlack(Collection<LfBus> buses, LoadFlowParameters.BalanceType balanceType, boolean useActiveLimits, boolean slackDistributionGeneratorsVoltageControlOnly) {
        ...
        ActivePowerDistribution activePowerDistribution = ActivePowerDistribution.create(
                balanceType,
                false,
                useActiveLimits,
    ...
    public DcLoadFlowResult run() {
        ...
        if (parameters.isDistributedSlack()) {
            distributeSlack(network.getBuses(),
                    parameters.getBalanceType(),
                    parameters.getNetworkParameters().isUseActiveLimits(),
                    parameters.getNetworkParameters().isSlackDistributionGeneratorsVoltageControlOnly()
            );
        }
