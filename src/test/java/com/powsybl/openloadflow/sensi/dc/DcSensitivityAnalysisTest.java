...
class DcSensitivityAnalysisTest extends AbstractSensitivityAnalysisTest {
    ...
    void test4busesDistributedPartialFactors() {
        ...
        SensitivityFactorsProvider factorsProvider = new SensitivityFactorsProvider() {
            @Override
            public List<SensitivityFactor> getFactors(Network network) {
                return createFactorMatrix(network.getGeneratorStream().filter(gen -> gen.getId().equals("g1")).collect(Collectors.toList()),
                        network.getBranchStream().collect(Collectors.toList()));
            }
        ...
        SensitivityFactorsProvider factorsProvider = new SensitivityFactorsProvider() {
            ...
            public List<SensitivityFactor> getFactors(Network network) {
                return network.getBranchStream()
                        .map(branch -> new BranchFlowPerLinearGlsk(new BranchFlow(branch.getId(), branch.getId(), branch.getId()), linearGlsk)).collect(Collectors.toList());
            }
    ...
    void testPhaseShifter() {
        ...
        SensitivityFactorsProvider factorsProvider = new SensitivityFactorsProvider() {
            @Override
            public List<SensitivityFactor> getFactors(Network network) {
                return Collections.singletonList(new BranchFlowPerPSTAngle(createBranchFlow(l1),
