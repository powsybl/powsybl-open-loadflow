...
class EquationSystemTest {
    ...
    void test() {
        ...
        LfNetwork network = lfNetworks.stream().filter(n -> n.getNum() == ComponentConstants.MAIN_NUM).findAny().orElse(null);
        ...
        clearEvents();
        EquationTerm equationTerm = EquationTerm.createVariableTerm(bus, VariableType.BUS_V, variableSet, bus.getV().eval());
